...
class UpdateEventAgent(ScAgentClassic):
    ...
    def run(self, action_node: ScAddr) -> ScResult:
        ...
        try:
            ...
            if not check_connector(
                ...
                self.logger.info("The message isn’t about updating calendar event")
                return ScResult.OK

            rrel_event_summary = ScKeynodes.resolve(
                "rrel_event_summary", sc_type.CONST_NODE_ROLE
            )
            ...
            summary_content = self.get_event_summary_content(
                message_addr, rrel_event_summary
            )
            ...
            rrel_new_event_summary = ScKeynodes.resolve(
                "rrel_new_event_summary", sc_type.CONST_NODE_ROLE
            )
            new_summary_link = search_element_by_role_relation(
                message_addr, rrel_new_event_summary
            )
            if new_summary_link:
                new_summary_content = get_link_content_data(new_summary_link)
                update_response = self.update_event(
                    headers, event["id"], new_summary=new_summary_content
                )
            rrel_start_time = ScKeynodes.resolve(
                "rrel_start_time", sc_type.CONST_NODE_ROLE
            )
            start_time_link = search_element_by_role_relation(
                message_addr, rrel_start_time
            )
            ...
            if start_time_link and end_time_link:
                ...
                start_time_content = get_link_content_data(start_time_link)
                update_response = self.update_event(
                    headers, event["id"], new_start_time=start_time_content
                )
            elif end_time_link:
                end_time_content = get_link_content_data(end_time_link)
                update_response = self.update_event(
                    headers, event["id"], new_end_time=end_time_content
                )
            else:
            ...
            if new_summary:
                ...
                if new_end_time:
                    update_data = {
                        ...
                        "end": {"dateTime": new_end_time, "timeZone": "Europe/Moscow"},
                    }
                else:
                    update_data = {
                        "start": {
    ...
    def get_authenticated_token(self) -> str | None:
        ...
        action, is_successful = execute_agent(
            arguments={current_user_node: False},

...
class CheckGoogleTokenAgent(ScAgentClassic):
    ...
    def on_event(
        ...
        refresh_token_link = search_element_by_non_role_relation(
            author, self.nrel_refresh_token
        )
        ...
        if not self.is_valid(access_token):
            ...
            if response.success is not True:
                self.logger.error(
                    f"Get {response.error}", f"{response.error_description}"
                )
                return ScResult.ERROR
            new_access_token = response.access_token
            new_link_content = ScLinkContent(
                new_access_token, ScLinkContentType.STRING, access_token_link
            )
    ...
    def refresh_access_token(self, refresh_token: str) -> RefreshResponse:
        try:
            ...
            return RefreshResponse(success=False, error=f"Request failed: {str(e)}")
        except KeyError:
            return RefreshResponse(success=False, error="Invalid response format")

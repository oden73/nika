...
from modules.yandex.yandex_disc.services import YandexDiskInfoService
...
class YandexDiscInfoAgent:
    def __init__(self):
        self.logger = logging.getLogger(self.__class__.__name__)

    def run(self, user_node: ScAddr, token: str) -> bool:
        try:
            info_text = YandexDiskInfoService.get_disk_info(token)
            self._save_to_kb(user_node, info_text)
            return True
        except Exception as e:
            self.logger.error(f"Failed to update disk info: {e}")
            return False

    def _save_to_kb(self, user_node: ScAddr, text: str):
        nrel_target = ScKeynodes.resolve("nrel_yandex_disc_info", sc_type.CONST_NODE_NON_ROLE)
